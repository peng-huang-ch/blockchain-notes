...
async function deploy({
	...
	var saltNonce = 1655775220144 || Date.now();
	...
	const ep = ethers.providers.getDefaultProvider(provider);
	let { maxFeePerGas, maxPriorityFeePerGas, gasPrice } = await ep.getFeeData();
	...
	console.log('tx data   : ', tx.toJSON())
	console.log('tx serialized		: ', tx.serialize().toString('hex'));
	console.log('message to sign    : ', tx.getMessageToSign(true).toString('hex'));
	// return;
